...
def duration_distribution(df, bin_size=5, max_limit=300):
    """Compute histogram distribution of durations."""
...
def save_dataset(df, jsonl_file, parquet_file, output_dir, prefix, suffix):
    ...
    elif parquet_file:  # only save parquet if jsonl not used
...
def generate_stats(df, output_dir, prefix, total_loaded, top_n=50):
    ...
    if "duration" in df.columns:
        ...
        if durations:
            stats["duration_stats_full"] = {
                "distribution": duration_distribution(df),
                "min": min(durations),
                "max": max(durations),
...
def main():
    ...
    # Load dataset (prefer JSONL if both given)
    ...
    # Stats + plots
    generate_stats(df, args.output_dir, prefix, total_loaded)
